...
public static class OtlpHttpEndpointsBuilder
{
    ...
    private static async Task<T> ReadOtlpData<T>(HttpContext httpContext, Func<ReadOnlySequence<byte>, T> exporter)
    {
        ...
        try
        {
            do
            {
                ...
                {
                    throw new OperationCanceledException(message: "Read call was canceled.");
                }

                if (result.Buffer.Length > MaxRequestSize)
                {
                    // Too big!
                    throw new BadHttpRequestException(message: $"The request body was larger than the max allowed of {MaxRequestSize} bytes.", statusCode: StatusCodes.Status400BadRequest);
                }
                ...
                {
                    httpContext.Request.BodyReader.AdvanceTo(consumed: result.Buffer.Start, examined: result.Buffer.End);
                }
            } while (true);
        ...
        finally
        {
            ...
            {
                httpContext.Request.BodyReader.AdvanceTo(consumed: result.Buffer.End);
            }
        }
    }
    ...
    {
        public static OtlpResult<T> Response<T>(T response) where T : IMessage
        {
            return new OtlpResult<T>(response);
        }
    }
